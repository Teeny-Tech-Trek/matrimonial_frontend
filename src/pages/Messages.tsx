...
const Messages: React.FC<MessagesProps> = ({ onNavigate }) => {
  ...
  // ✅ FIXED: Fetch connections and extract proper names
  useEffect(() => {
    const fetchData = async () => {
      setIsLoading(true);
      try {
        ...
        // ✅ FIX: Extract proper user data with ALL possible name fields
        // const processedConnections = connectionsData.data.map((conn: any) => {
        //   console.log("🔍 Processing connection:", conn);
          
        //   // Try to find the other user (not current user)
        //   let otherUser = null;
          
        //   // Case 1: Connection has sender/receiver
        //   if (conn.sender && conn.receiver) {
        //     otherUser = conn.sender._id === userId || conn.sender.userId === userId 
        //       ? conn.receiver 
        //       : conn.sender;
        //   }
        //   // Case 2: Connection has userId field
        //   else if (conn.userId) {
        //     otherUser = conn;
        //   }
        //   // Case 3: Connection is the user object itself
        //   else {
        //     otherUser = conn;
        //   }
        ...
        //     otherUser?.fullName || 
        //     otherUser?.name || 
        //     otherUser?.sender?.fullName ||
        //     otherUser?.receiver?.fullName ||
        //     conn?.fullName ||
        //     conn?.name ||
        //     conn?.sender?.fullName ||
        //     conn?.receiver?.fullName ||
        //     "Unknown User";
        ...
        //   console.log("✅ Extracted data:", { name, userId, avatar });
          
        //   return {
        //     _id: userId,
        //     fullName: name,
        //     avatar: avatar,
        //     profilePhotos: otherUser?.profilePhotos || conn?.profilePhotos || [],
        //   };
        // });
      const processedConnections = connectionsData.data.map((conn: any) => {
  let otherUser = null;
  ...
  return {
    ...
    avatar: otherUser.avatar || otherUser.profilePhotos?.[0] || null,
  };
});

console.log("✅ Final Processed Connections:", processedConnections);
        ...
        console.log(JSON.stringify(connectionsData.data, null, 2));
        ...
        <div className="flex-1 overflow-y-auto">
          {connections.length === 0 ? (
            ...
            connections.map((conn) => {
              ...
              console.log(`📊 Sidebar - Name: ${conn.fullName}, Unread: ${unread}`);

              // ✅ Use random default avatar if no avatar
              const avatarUrl = conn.avatar || getDefaultAvatar(conn._id);
          ...
          <>
            <div className="flex items-center gap-3 px-6 py-4 border-b border-gray-200 bg-white flex-shrink-0">
              {(() => {
                ...
                const avatarUrl = otherUser?.avatar || getDefaultAvatar(otherUser?._id || "default");
